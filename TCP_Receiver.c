...
int main(int argc, char *argv[]) {
    ...
    int sock_client = accept(sock, (struct sockaddr *)&client, &client_len);
    if (sock_client == -1) {
        perror("accept");
        exit(1);
    }
    ...
    do {
        ...
        // TODO: understand how to receive bytes that went missing
        // do {
        ...
        runs[times].id = times;
        runs[times].elapsed_time = ((end_time.tv_sec - start_time.tv_sec) * 1000000.0 + (end_time.tv_usec - start_time.tv_usec)) / 1000.0;
        runs[times].speed = strlen(buffer)/(runs[times].elapsed_time*0.001);
